...
<template>
    <div class="flex flex-col gap-3 rounded-2xl bg-white p-6">
        <div>
            ...
            <p class="mt-0.5 text-xs text-stone-400">Notez le déroulement du cours, les points abordés et toute remarque utile.</p>
        </div>
        <textarea
        ...
        <div class="flex items-center justify-between gap-3">
            <div class="flex min-w-0 flex-col gap-0.5">
                <label class="flex cursor-pointer items-center gap-1.5" title="Sauvegarder automatiquement à chaque frappe">
                    <input
                        v-model="autoSave"
                        type="checkbox"
                    ...
                    <span class="text-[11px] text-stone-400">
                        <abbr title="Sauvegarde automatique">Save auto</abbr></span>
                </label>
                <span v-if="form.processing" class="text-[10px] text-stone-400">Sauvegarde…</span>
                <span v-else-if="lessonNote?.savedAt" class="truncate text-[10px] text-stone-400">{{ lessonNote.savedAt }}</span>
            </div>
            <Button
                variant="primary"
                size="sm"
