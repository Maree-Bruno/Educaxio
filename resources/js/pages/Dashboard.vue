...
function statCards(school: School) {
    return [
        ...
        { label: 'Professeurs',               value: school.stats.teachers, highlight: false },
        { label: "En attente d'adhésion",     value: school.stats.pending,  highlight: school.stats.pending > 0 },
...
<template>
    ...
    <div v-for="school in schools" :key="school.id" class="flex flex-col gap-8">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            >
                <span class="text-3xl font-extrabold" :class="stat.highlight ? 'text-orange' : 'text-text-base'">{{ stat.value }}</span>
                <span class="text-xs font-bold uppercase tracking-wider text-border-figma">{{ stat.label }}</span>
            </div>
        </div>
                    ...
                    <ul v-else class="divide-y divide-neutral-100 bg-white">
                        ...
                        >
                            ...
                            <div class="flex shrink-0 gap-2">
                                <Button variant="primary" size="sm" label="Approuver" :loading="loadingApprove[req.id]" @click="approveRequest(school, req)" />
                                <Button variant="danger"  size="sm" label="Refuser"   :loading="loadingReject[req.id]"  @click="rejectRequest(school, req)" />
                            </div>
                        </li>
                    </ul>
                </DashboardCard>
                ...
                >
                    ...
                    <template v-else>
                        ...
                        <div class="bg-gray-100 px-6 py-3">
                            <Pagination :links="school.recentStudents.links" :current-page="school.recentStudents.current_page" :last-page="school.recentStudents.last_page" />
                        </div>
                    </template>
                </DashboardCard>
            ...
            <div class="flex flex-col gap-8">
                ...
                >
                    ...
                    <template v-else>
                        ...
                        <div class="bg-gray-100 px-6 py-3">
                            <Pagination :links="school.teachers.links" :current-page="school.teachers.current_page" :last-page="school.teachers.last_page" />
                        </div>
                    </template>
                </DashboardCard>

                <!-- Accès rapide -->
                <DashboardCard title="Accès rapide" description="Raccourcis vers les principales sections de gestion.">
